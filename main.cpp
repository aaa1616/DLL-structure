...
int main()
{
	...
	HMODULE hdll;
	char *processname = "Singleton.exe";
	...
	//LPCWSTR inject_filename = _T("D:\\Documents\\visual studio 2015\\Projects\\Test\\CallDotNetDll\\MidDll.dll");
	LPCWSTR inject_filename = _T("D:\\Documents\\visual studio 2015\\Projects\\Test\\Debug\\CloseSingletonDLL.dll");
	//LPCSTR funcname = "add";
	LPCSTR inject_funcname = "closehandle";
    ...
    pFun = (LPTHREAD_START_ROUTINE)GetProcAddress(hdll, inject_funcname);
    if (pFun == NULL) {
    ...
    printf("WriteRrmoyrProcess Size is %d\n", Size);
	if ((hThread = CreateRemoteThread(hRemoteProcess, NULL, 0, pFun, NULL, 0, &dwThreadId)) == NULL) {
	...
	UninjectDLL(processid, hRemoteProcess, _T("CloseSingletonDLL.dll"));
